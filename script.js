...
var wordsAdded = ["JAVASCRIPT", "ORACLE", "ANDROID", "HTML", "ALURA", "CSS", "NODE", "FRAMEWORK", "JAVA", "GIT", "WINDOWS", "MAC", "INTERNET", "TCP", "DNS", "HTTP", "LINUX", "BOOTSTRAP", "FRONTEND", "BACKEND"];
const tentativas = 6;
...
function compareLists(letter) {
    ...
    if (position < 0) {
        ...
        if (tentativasRestantes === 0) {
            let messageBody = `Tenta de novo, na próxima você acerta! <br> A palavra secreta era: <br><strong>${secretWord}</strong>`;
            let messageTitle = "Poxa que pena, você errou!  TT" ;
    ...
    if (victory) {
        let messageBody = "Tente de novo e veja se não é sorte de principiante.";
        let messageTitle = "Parabéns, você acertou!";
        openModal(messageBody, messageTitle);
